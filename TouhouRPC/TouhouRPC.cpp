...
void programClose() {
    Log::info("User asked to close the program. Exiting...");
}


BOOL WINAPI ConsoleHandlerRoutine(DWORD dwCtrlType) {
    if (static_cast<int>(dwCtrlType) == CTRL_CLOSE_EVENT) {
        programClose();
        return TRUE;
    }
    return FALSE;
...
int main() {
    ...
    Log::info("Terminating program (reaching the end of the code).");
}