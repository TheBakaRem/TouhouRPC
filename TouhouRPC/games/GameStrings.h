#include <string>
#pragma once

// Touhou 06
const std::string th06_musicNames[17] = {
	"A Dream More Scarlet than Red",
	"A Soul as Scarlet as a Ground Cherry",
	"Apparitions Stalk the Night",
	"Lunate Elf",
	"Tomboyish Girl in Love",
	"Shanghai Scarlet Teahouse ~ Chinese Tea",
	"Shanghai Alice of Meiji 17",
	"Voile, the Magic Library",
	"Locked Girl ~ The Girl's Sealed Room",
	"The Maid and the Pocket Watch of Blood",
	"Lunar Clock ~ Luna Dial",
	"The Young Descendant of Tepes",
	"Septette for a Dead Princess",
	"The Centennial Festival for Magical Girls",
	"U.N. Owen was Her?",
	"An Eternity More Transient than Scarlet",
	"Scarlet Tower ~ Eastern Dream...",
};

// Touhou 07
const std::string th07_musicNames[20] = {
	"Ghostly Dream ~ Snow or Cherry Petal",
	"Paradise ~ Deep Mountain",
	"Crystallized Silver",
	"The Fantastic Tales from Tono",
	"Diao Ye Zong (withered leaf)",
	"The Doll Maker of Bucuresti",
	"Doll Judgement ~ The Girl Who Played with People's Shapes",
	"The Capital City of Flowers in the Sky",
	"Phantom Band ~ Phantom Ensemble",
	"Eastern Ghostly Dream ~ Ancient Temple",
	"Hiroari Shoots a Strange Bird ~ Till When?",
	"Ultimate Truth",
	"Bloom Nobly, Ink-Black Cherry Blossom ~ Border of Life",
	"Border of Life",
	"Spiritual Domination",
	"A Maiden's Illusionary Funeral ~ Necro-Fantasy",
	"Spiritual Domination ~ Who done it!",
	"Necrofantasia",
	"Dream of a Spring Breeze",
	"Sakura, Sakura ~ Japanize Dream...",
};

// Touhou 08
const std::string th08_musicNames[21] = {
	"Eternal Night Vignette ~ Eastern Night",
	"Illusionary Night ~ Ghostly Eyes",
	"Stirring an Autumn Moon ~ Mooned Insect",
	"Song of the Night Sparrow ~ Night Bird",
	"Deaf to All but the Song",
	"Nostalgic Blood of the East ~ Old World",
	"Plain Asia",
	"Retribution for the Eternal Night ~ Imperishable Night",
	"Maiden's Capriccio ~ Dream Battle",
	"Love-Colored Master Spark",
	"Cinderella Cage ~ Kagome-Kagome",
	"Lunatic Eyes ~ Invisible Full Moon",
	"Voyage 1969",
	"Gensokyo Millennium ~ History of the Moon",
	"Flight of the Bamboo Cutter ~ Lunatic Princess",
	"Voyage 1970",
	"Extend Ash ~ Person of Hourai",
	"Reach for the Moon, Immortal Smoke",
	"Evening Primrose",
	"Eternal Dream ~ Mystical Maple",
	"Eastern Youkai Beauty",
};

// Touhou 09
const std::string th09_musicNames[19] = {
	"Flower Reflecting Mound ~ Higan Retour",
	"Spring Lane ~ Colorful Path",
	"Oriental Dark Flight",
	"Flowering Night",
	"Eastern Ghostly Dream ~ Ancient Temple",
	"Lunatic Eyes ~ Invisible Full Moon",
	"Adventure of the Lovestruck Tomboy",
	"Phantom Band ~ Phantom Ensemble",
	"Deaf to All but the Song ~ Flower Mix",
	"White Flag of Usa Shrine",
	"Wind God Girl",
	"Poison Body ~ Forsaken Doll",
	"Gensokyo, Past and Present ~ Flower Land",
	"Higan Retour ~ Riverside View",
	"Eastern Judgement in the Sixtieth Year ~ Fate of Sixty Years",
	"The Mound where the Flowers Reflect",
	"Mound of Shigan",
	"The Flowers Remain in Fantasy",
	"Flower of Soul ~ Another Dream...",
};

const std::string th09_stageNames[14] = {
	"Bamboo Forest of the Lost",
	"Bamboo Forest of the Lost",
	"Phantom Meadow",
	"Hakugyokurou's Stairway",
	"Eientei",
	"Misty Lake",
	"Barrier of Life & Death",
	"Youkai Trail",
	"Bamboo Forest of the Lost",
	"Garden of the Sun",
	"Giant Toad's Pond",
	"The Nameless Hill",
	"Road of Reconsideration",
	"Muenzuka",
};

const std::string th09_characterNames[16] = {
	"Reimu",
	"Marisa",
	"Sakuya",
	"Youmu",
	"Reisen",
	"Cirno",
	"Lyrica",
	"Mystia",
	"Tewi",
	"Yuuka",
	"Aya",
	"Medicine",
	"Komachi",
	"Eiki",
	"Merlin",
	"Lunasa",
};

// Touhou 08
const std::string th08_spellCardName[222] = {
	// stage 1 (13 cards)
	"Firefly Sign \"Meteors on Earth\"",
	"Firefly Sign \"Comets on Earth\"",
	"Lamp Sign \"Firefly Phenomenon\"",
	"Lamp Sign \"Firefly Phenomenon\"",
	"Lamp Sign \"Firefly Phenomenon\"",
	"Lamp Sign \"Firefly Phenomenon\"",
	"Wriggle Sign \"Little Bug\"",
	"Wriggle Sign \"Little Bug Storm\"",
	"Wriggle Sign \"Night Bug Storm\"",
	"Wriggle Sign \"Night Bug Tornado\"",
	"Hidden Bug \"Endless Night Seclusion\"",
	"Hidden Bug \"Endless Night Seclusion\"",
	"Hidden Bug \"Endless Night Seclusion\"",
	// stage 2 (19 cards)
	"Vocal Sign \"Hooting in the Night\"",
	"Vocal Sign \"Hooting in the Night\"",
	"Vocal Sign \"Howl of the Horned Owl\"",
	"Vocal Sign \"Howl of the Horned Owl\"",
	"Moth Sign \"Hawk Moth's Insect Curse\"",
	"Moth Sign \"Hawk Moth's Insect Curse\"",
	"Toxin Sign \"Poisonous Moth's Scales\"",
	"Deadly Toxin \"Poisonous Moth's Dance in the Dark\"",
	"Hawk Sign \"Ill - Starred Dive\"",
	"Hawk Sign \"Ill - Starred Dive\"",
	"Hawk Sign \"Ill - Starred Dive\"",
	"Hawk Sign \"Ill - Starred Dive\"",
	"Night-Blindness \"Song of the Night Sparrow\"",
	"Night-Blindness \"Song of the Night Sparrow\"",
	"Night-Blindness \"Song of the Night Sparrow\"",
	"Night-Blindness \"Song of the Night Sparrow\"",
	"Night Sparrow \"Midnight Chorus Master\"",
	"Night Sparrow \"Midnight Chorus Master\"",
	"Night Sparrow \"Midnight Chorus Master\"",
	// stage 3 (22 cards)
	"Spiritual Birth \"First Pyramid\"",
	"Spiritual Birth \"First Pyramid\"",
	"Spiritual Birth \"First Pyramid\"",
	"Spiritual Birth \"First Pyramid\"",
	"Origin Sign \"Ephemerality 137\"",
	"Origin Sign \"Ephemerality 137\"",
	"Origin Sign \"Ephemerality 137\"",
	"Ambition Sign \"Buretsu Crisis\"",
	"Ambition Sign \"Masakado Crisis\"",
	"Ambition Sign \"Yoshimitsu Crisis\"",
	"Ambition Sign \"General Headquarters Crisis\"",
	"Land Sign \"Three Sacred Treasures - Sword\"",
	"Land Sign \"Three Sacred Treasures - Orb\"",
	"Land Sign \"Three Sacred Treasures - Mirror\"",
	"Land Scheme \"Three Sacred Treasures - Country\"",
	"Ending Sign \"Phantasmal Emperor\"",
	"Ending Sign \"Phantasmal Emperor\"",
	"Pseudo-History \"The Legend of Gensokyo\"",
	"Pseudo-History \"The Legend of Gensokyo\"",
	"Future \"Gods' Realm\"",
	"Future \"Gods' Realm\"",
	"Future \"Gods' Realm\"",
	// stage 4A (23 cards)
	"Dream Sign \"Duplex Barrier\"",
	"Dream Sign \"Duplex Barrier\"",
	"Dream Sign \"Great Duplex Barrier\"",
	"Dream Sign \"Great Duplex Barrier\"",
	"Spirit Sign \"Fantasy Seal -Spread-\"",
	"Spirit Sign \"Fantasy Seal -Spread-\"",
	"Scattered Spirit \"Fantasy Seal -Worn-\"",
	"Scattered Spirit \"Fantasy Seal -Worn-\"",
	"Dream Sign \"Evil - Sealing Circle\"",
	"Dream Sign \"Evil - Sealing Circle\"",
	"Divine Arts \"Omnidirectional Oni - Binding Circle\"",
	"Divine Arts \"Omnidirectional Oni - Binding Circle\"",
	"Spirit Sign \"Fantasy Seal -Concentrate-\"",
	"Spirit Sign \"Fantasy Seal -Concentrate-\"",
	"Migrating Spirit \"Fantasy Seal -Marred-\"",
	"Migrating Spirit \"Fantasy Seal -Marred-\"",
	"Boundary \"Duplex Danmaku Barrier\"",
	"Boundary \"Duplex Danmaku Barrier\"",
	"Great Barrier \"Hakurei Danmaku Barrier\"",
	"Great Barrier \"Hakurei Danmaku Barrier\"",
	"Divine Spirit \"Fantasy Seal -Blink-\"",
	"Divine Spirit \"Fantasy Seal -Blink-\"",
	"Divine Spirit \"Fantasy Seal -Blink-\"",
	// stage 4B (23 cards)
	"Magic Sign \"Milky Way\"",
	"Magic Sign \"Milky Way\"",
	"Magic Space \"Asteroid Belt\"",
	"Magic Space \"Asteroid Belt\"",
	"Magic Sign \"Stardust Reverie\"",
	"Magic Sign \"Stardust Reverie\"",
	"Black Magic \"Event Horizon\"",
	"Black Magic \"Event Horizon\"",
	"Love Sign \"Non - Directional Laser\"",
	"Love Sign \"Non - Directional Laser\"",
	"Love Storm \"Starlight Typhoon\"",
	"Love Storm \"Starlight Typhoon\"",
	"Love Sign \"Master Spark\"",
	"Love Sign \"Master Spark\"",
	"Loving Heart \"Double Spark\"",
	"Loving Heart \"Double Spark\"",
	"Light Sign \"Earthlight Ray\"",
	"Light Sign \"Earthlight Ray\"",
	"Light Blast \"Shoot the Moon\"",
	"Light Blast \"Shoot the Moon\"",
	"Magicannon \"Final Spark\"",
	"Magicannon \"Final Spark\"",
	"Magicannon \"Final Master Spark\"",
	// stage 5 (19 cards)
	"Wave Sign \"Red-Eyed Hypnosis (Mind Shaker)\"",
	"Wave Sign \"Red-Eyed Hypnosis (Mind Shaker)\"",
	"Illusion Wave \"Red-Eyed Hypnosis (Mind Blowing)\"",
	"Illusion Wave \"Red-Eyed Hypnosis (Mind Blowing)\"",
	"Lunatic Sign \"Hallucinogenic Tuning (Visionary Tuning)\"",
	"Lunatic Sign \"Hallucinogenic Tuning (Visionary Tuning)\"",
	"Lunatic Gaze \"Lunatic Stare Tuning (Illusion Seeker)\"",
	"Lunatic Gaze \"Lunatic Stare Tuning (Illusion Seeker)\"",
	"Loafing Sign \"Life and Spirit Stopping (Idling Wave)\"",
	"Loafing Sign \"Life and Spirit Stopping (Idling Wave)\"",
	"Indolence \"Life and Spirit Stopping (Mind Stopper)\"",
	"Indolence \"Life and Spirit Stopping (Mind Stopper)\"",
	"Spread Sign \"Moon of Truth (Invisible Full Moon)\"",
	"Spread Sign \"Moon of Truth (Invisible Full Moon)\"",
	"Spread Sign \"Moon of Truth (Invisible Full Moon)\"",
	"Spread Sign \"Moon of Truth (Invisible Full Moon)\"",
	"Lunar Eyes \"Lunar Rabbit's Remote Mesmerism (Tele-Mesmerism)\"",
	"Lunar Eyes \"Lunar Rabbit's Remote Mesmerism (Tele-Mesmerism)\"",
	"Lunar Eyes \"Lunar Rabbit's Remote Mesmerism (Tele-Mesmerism)\"",
	// stage 6A (28 cards)
	"Spacesphere \"Earth in a Pot\"",
	"Spacesphere \"Earth in a Pot\"",
	"Spacesphere \"Earth in a Pot\"",
	"Spacesphere \"Earth in a Pot\"",
	"Awakened God \"Memories of the Age of the Gods\"",
	"Awakened God \"Memories of the Age of the Gods\"",
	"God Sign \"Genealogy of the Celestials\"",
	"God Sign \"Genealogy of the Celestials\"",
	"Revival \"Seimei Yugi -Life Game- \"",
	"Revival \"Seimei Yugi -Life Game- \"",
	"Resurrection \"Rising Game\"",
	"Resurrection \"Rising Game\"",
	"Leading God \"Omoikane's Device\"",
	"Leading God \"Omoikane's Device\"",
	"Mind of God \"Omoikane's Brain\"",
	"Mind of God \"Omoikane's Brain\"",
	"Curse of the Heavens \"Apollo 13\"",
	"Curse of the Heavens \"Apollo 13\"",
	"Curse of the Heavens \"Apollo 13\"",
	"Curse of the Heavens \"Apollo 13\"",
	"Esoterica \"Astronomical Entombing\"",
	"Esoterica \"Astronomical Entombing\"",
	"Esoterica \"Astronomical Entombing\"",
	"Esoterica \"Astronomical Entombing\"",
	"Forbidden Elixir \"Hourai Elixir\"",
	"Forbidden Elixir \"Hourai Elixir\"",
	"Forbidden Elixir \"Hourai Elixir\"",
	"Forbidden Elixir \"Hourai Elixir\"",
	// stage 6B (44 cards)
	"Medicine Sign \"Galaxy in a Pot\"",
	"Medicine Sign \"Galaxy in a Pot\"",
	"Medicine Sign \"Galaxy in a Pot\"",
	"Medicine Sign \"Galaxy in a Pot\"",
	"Impossible Request \"Jewel from the Dragon's Neck -Five-Colored Shots-\"",
	"Impossible Request \"Jewel from the Dragon's Neck -Five-Colored Shots-\"",
	"Divine Treasure \"Divine Dragon Bullet\"",
	"Divine Treasure \"Divine Dragon Bullet\"",
	"Impossible Request \"Buddha's Stone Bowl -Indomitable Will-\"",
	"Impossible Request \"Buddha's Stone Bowl -Indomitable Will-\"",
	"Divine Treasure \"Buddhist Diamond\"",
	"Divine Treasure \"Buddhist Diamond\"",
	"Impossible Request \"Robe of Fire Rat -Unhurried Mind-\"",
	"Impossible Request \"Robe of Fire Rat -Unhurried Mind-\"",
	"Divine Treasure \"Salamander Shield\"",
	"Divine Treasure \"Salamander Shield\"",
	"Impossible Request \"Swallow's Cowrie Shell -Everlasting Life-\"",
	"Impossible Request \"Swallow's Cowrie Shell -Everlasting Life-\"",
	"Divine Treasure \"Life Spring Infinity\"",
	"Divine Treasure \"Life Spring Infinity\"",
	"Impossible Request \"Bullet Branch of Hourai -Rainbow Danmaku -\"",
	"Impossible Request \"Bullet Branch of Hourai -Rainbow Danmaku -\"",
	"Divine Treasure \"Jeweled Branch of Hourai -Dreamlike Paradise-\"",
	"Divine Treasure \"Jeweled Branch of Hourai -Dreamlike Paradise-\"",
	"\"End of Imperishable Night -New Moon-\"",
	"\"End of Imperishable Night -Crescent Moon-\"",
	"\"End of Imperishable Night -First Quarter's Moon-\"",
	"\"End of Imperishable Night -Matsuyoi-\"",
	"\"End of Imperishable Night -11 o'Clock-\"",
	"\"End of Imperishable Night -Half to Midnight-\"",
	"\"End of Imperishable Night -Midnight-\"",
	"\"End of Imperishable Night -Half Past Midnight-\"",
	"\"End of Imperishable Night -1 o'Clock-\"",
	"\"End of Imperishable Night -Half Past 1-\"",
	"\"End of Imperishable Night -Dead of Night-\"",
	"\"End of Imperishable Night -Half Past 2-\"",
	"\"End of Imperishable Night -3 o'Clock-\"",
	"\"End of Imperishable Night -Half Past 3-\"",
	"\"End of Imperishable Night -4 o'Clock-\"",
	"\"End of Imperishable Night -Half Past 4-\"",
	"\"End of Imperishable Night -Morning Mist-\"",
	"\"End of Imperishable Night -Dawn-\"",
	"\"End of Imperishable Night -Morning Star-\"",
	"\"End of Imperishable Night -Rising World-\"",
	// extra stage (14 cards)
	"Past \"Old History of an Untrodden Land -Old History-\"",
	"Reincarnation \"Ichijou Returning Bridge\"",
	"Future \"New History of Fantasy -Next History-\"",
	"Limiting Edict \"Curse of Tsuki-no-Iwakasa\"",
	"Undying \"Fire Bird -Feng Wing Ascension-\"",
	"Fujiwara \"Wounds of Metsuzai Temple\"",
	"Undying \"Xu Fu's Dimension\"",
	"Expiation \"Honest Man's Death\"",
	"Hollow Being \"Wu\"",
	"Inextinguishable \"Phoenix's Tail\"",
	"Hourai \"South Wind, Clear Sky -Fujiyama Volcano-\"",
	"\"Possessed by Phoenix\"",
	"\"Hourai Doll\"",
	"\"Imperishable Shooting\"",
	// last words (17 cards)
	"\"Unseasonal Butterfly Storm\"",
	"\"Blind Nightbird\"",
	"\"Emperor of the Land of the Rising Sun\"",
	"\"Stare of the Hazy Phantom Moon (Lunatic Red Eyes)\"",
	"\"Heaven Spider's Butterfly-Capturing Web\"",
	"\"Tree-Ocean of Hourai\"",
	"\"Phoenix Rebirth\"",
	"\"Ancient Duper\"",
	"\"Total Purification\"",
	"\"Fantasy Nature\"",
	"\"Blazing Star\"",
	"\"Deflation World\"",
	"\"Matsuyoi-Reflecting Satellite Slash\"",
	"\"The Phantom of the Grand Guignol\"",
	"\"Scarlet Destiny\"",
	"\"Saigyouji Parinirvana\"",
	"\"Profound Danmaku Barrier -Phantasm, Foam and Shadow-\""
};

// Touhou 09.5 Bosses and Spells
// Empty strings are for unused levels.
const std::string th095_bossAndSpells[110] = {
	// Level 1
	"Wriggle Nightbug",
	"Rumia",
	"Wriggle Nightbug - Firefly Sign \"Fixed Stars on Earth\"",
	"Rumia - Darkness Sign \"Dark Side of the Moon\"",
	"Wriggle Nightbug - Butterfly Sign \"Butterfly Storm\"",
	"Rumia - Night Sign \"Midnight Bird\"",
	"",
	"",
	"",
	"",
	// Level 2
	"Cirno",
	"Letty Whiterock",
	"Cirno - Snow Sign \"Diamond Blizzard\"",
	"Letty Whiterock - Cold Sign \"Cold Snap\"",
	"Cirno - Freeze Sign \"Minus K\"",
	"Letty Whiterock - Winter Sign \"Northern Winner\"",
	"",
	"",
	"",
	"",
	// Level 3
	"Alice Margatroid",
	"Keine Kamishirasawa - Light Sign \"Amaterasu\"",
	"Alice Margatroid - Puppeteer Sign \"Dolls in Sea\"",
	"Keine Kamishirasawa - Shroud Sign \"Rain of Showa\"",
	"Alice Margatroid - Curse Sign \"Straw Doll Kamikaze\"",
	"Keine Kamishirasawa - Hollyhock Sign \"Mito no Mitsukuni\"",
	"Alice Margatroid - Red Sign \"Doll Mira Ceti\"",
	"Keine Kamishirasawa - Japan Sign \"Yamato Kingdom\"",
	"",
	"",
	// Level 4
	"Reisen Udongein Inaba",
	"Medicine Melancholy - Fog Sign \"Gassing Garden\"",
	"Tewi Inaba - Runaway Rabbit \"Fluster Escape\"",
	"Reisen Udongein Inaba - Spread Sign \"Hazy Moon Flower Bookmark (Rocket in Mist)\"",
	"Medicine Melancholy - Poison Sign \"Poison Breath\"",
	"Reisen Udongein Inaba - Wave Sign \"Moon of Illusion (Invisible Half-Moon)\"",
	"Medicine Melancholy - Confusion \"Into Delirium\"",
	"Tewi Inaba - Owing Sign \"Vulnerary of Oonamuchi-sama\"",
	"Reisen Udongein Inaba - Mad Dream \"Dream of Insanity (Dream World)\"",
	"",
	// Level 5
	"Hong Meiling",
	"Patchouli Knowledge - Sun & Water Sign \"Hydrogenous Prominence\"",
	"Hong Meiling - Flower Sign \"Colorful Light Lotus Flower Palm\"",
	"Patchouli Knowledge - Water & Fire Sign \"Phlogistic Rain\"",
	"Hong Meiling - Colorful Flip \"Fluttering Petals and Falling Leaves\"",
	"Patchouli Knowledge - Moon & Wood Sign \"Satellite Himawari\"",
	"Hong Meiling - Colorful Chinese Flower \"Rainbow Taijiquan\"",
	"Patchouli Knowledge - Sun & Moon Sign \"Royal Diamond Ring\"",
	"",
	"",
	// Level 6
	"Chen",
	"Youmu Konpaku - Human Knowledge Sword \"Turning Angel Cut\"",
	"Chen - Star Sign \"Leaping Kasane-Uroko Star\"",
	"Youmu Konpaku - Obsession Sword \"Blood of Asura\"",
	"Chen - Kishin \"Rumbling Jikokuten\"",
	"Youmu Konpaku - Celestial Star Sword \"As the Silent Nirvana\"",
	"Chen - Bakeneko \"Chen\"",
	"Youmu Konpaku - Four Births Sword \"Echoes of the Inclemency of All Beings\"",
	"",
	"",
	// Level 7
	"Sakuya Izayoi",
	"Remilia Scarlet - Devil Sign \"All the World in Nightmare\"",
	"Sakuya Izayoi - Time Sign \"Tunnel Effect\"",
	"Remilia Scarlet - Scarlet Sign \"Bloody Magic Square\"",
	"Sakuya Izayoi - Void \"Inflation Square\"",
	"Remilia Scarlet - Scarlet Bat \"Vampirish Night\"",
	"Sakuya Izayoi - Silver Sign \"Perfect Maid\"",
	"Remilia Scarlet - God Devil \"Remilia Stoker\"",
	"",
	"",
	// Level 8
	"Ran Yakumo",
	"Yuyuko Saigyouji - Ghostly Elegance \"Light Trap of Passing Away\"",
	"Ran Yakumo - Esoteric Sign \"Odaishi-sama's Secret Key\"",
	"Yuyuko Saigyouji - Butterfly Sign \"Deadly Lance of the Swallowtail Butterfly Crest\"",
	"Ran Yakumo - Ascetic Sign \"Eighty Million Goma Boards\"",
	"Yuyuko Saigyouji - Death Sign \"Life of Drunk, Dream of Death\"",
	"Ran Yakumo - Superman \"Soaring En no Ozunu\"",
	"Yuyuko Saigyouji - \"Death Butterfly, Floating Moon\"",
	"",
	"",
	// Level 9
	"Eirin Yagokoro",
	"Kaguya Houraisan - New Impossible Request \"Lunar Ilmenite\"",
	"Eirin Yagokoro - Medicine Sign \"Butterfly Dream Pill - Nightmare Type\"",
	"Kaguya Houraisan - New Impossible Request \"Red Stone of Aja\"",
	"Eirin Yagokoro - Elixir \"Mercury Sea\"",
	"Kaguya Houraisan - New Impossible Request \"Seamless Ceiling of Kinkaku-ji\"",
	"Eirin Yagokoro - Secret Elixir \"Lunar Incense\"",
	"Kaguya Houraisan - New Impossible Request \"Mysterium\"",
	"",
	"",
	// Level 10
	"Komachi Onozuka",
	"Eiki Shiki, Yamaxanadu - Lie \"Tongue of Wolf\"",
	"Komachi Onozuka - Death Song \"Ferry of the Deep Fog\"",
	"Eiki Shiki, Yamaxanadu - Judgment \"Trial of the Ten Kings\"",
	"Komachi Onozuka - Old Rain \"Rain in the Liminal Journey Through Yomi\"",
	"Eiki Shiki, Yamaxanadu - Judgment \"Guilty or Not Guilty\"",
	"Komachi Onozuka - Death Price \"Price of Life\"",
	"Eiki Shiki, Yamaxanadu - Judgment \"Cleansed Crystal Judgment -Aya Shameimaru-\"",
	"",
	"",
	// Level EX
	"Flandre Scarlet - Taboo \"Forbidden Fruit\"",
	"Flandre Scarlet - Taboo \"Forbidden Games\"",
	"Yukari Yakumo - Boundary Sign \"Boundary of Form and Emptiness\"",
	"Yukari Yakumo - Boundary Sign \"Boundary of Wave and Particle\"",
	"Fujiwara no Mokou - Exalted Personage \"Forewarning of St.Germain\"",
	"Fujiwara no Mokou - Hourai \"Mizunoe no Uranoshimako and Five-Colored Turtles\"",
	"Suika Ibuki - Ghastly Air \"Deep Fog Labyrinth\"",
	"Suika Ibuki - \"Night Parade of a Million Demons\"",
	"",
	"",
};


// Touhou 09.5 Music
const std::string th095_musicNames[6] = {
	"Tengu's Notebook ~ Mysterious Note",
	"Wind Circulation ~ Wind Tour",
	"Tengu is Watching ~ Black Eyes",
	"Sleepless Night of the Eastern Country",
	"Retrospective Kyoto",
	"Wind God Girl",
};

// Touhou 10
const std::string th10_musicNames[18] = {
	"A God That Misses People ~ Romantic Fall",
	"Because Princess Inada Is Scolding Me",
	"Sealed Gods",
	"The Road of the Misfortune God ~ Dark Road",
	"Dark Side of Fate",
	"The Gensokyo the Gods Loved",
	"Akutagawa Ryuunosuke's \"Kappa\" ~ Candid Friend",
	"Fall of Fall ~ Autumnal Waterfall",
	"Youkai Mountain ~ Mysterious Mountain",
	"The Primal Scene of Japan the Girl Saw",
	"Faith Is for the Transient People",
	"Cemetery of Onbashira ~ Grave of Being",
	"The Venerable Ancient Battlefield ~ Suwa Foughten Field",
	"Shrine at the Foot of the Mountain",
	"The Gods Give Us Blessed Rain ~ Sylphid Dream",
	"Tomorrow Will Be Special, Yesterday Was Not",
	"Native Faith",
	"Player's Score",
};

// Touhou 11
const std::string th11_musicNames[20] = {
	"Awakening of the Earth Spirits",
	"The Dark Blowhole",
	"The Sealed Youkai ~ Lost Place",
	"The Bridge People No Longer Cross",
	"unused",
	"Green-Eyed Jealousy",
	"Walking the Streets of a Former Hell",
	"A Flower-Studded Saké Dish on Mt. Ooe",
	"Heartfelt Fancy",
	"unused",
	"Satori Maiden ~ 3rd eye",
	"unused",
	"Lullaby of Deserted Hell",
	"Corpse Voyage ~ Be of good cheer!",
	"Hellfire Mantle",
	"Last Remote",
	"Solar Sect of Mystic Wisdom ~ Nuclear Fusion",
	"Hartmann's Youkai Girl",
	"The Earth Spirits' Homecoming",
	"Energy Daybreak ~ Future Dream...",
};

// Touhou 12
const std::string th12_musicNames[22] = {
	"At the End of Spring",
	"A Shadow in the Blue Sky",
	"A Tiny, Tiny, Clever Commander",
	"unused",
	"The Sealed Cloud Route",
	"Beware the Umbrella Left There Forever",
	"unused",
	"Sky Ruin",
	"The Traditional Old Man and the Stylish Girl",
	"Interdimensional Voyage of a Ghostly Passenger Ship",
	"Captain Murasa",
	"unused",
	"unused",
	"Rural Makai City Esoteria",
	"The Tiger-Patterned Bishamonten",
	"unused",
	"Fires of Hokkai",
	"Emotional Skyscraper ~ Cosmic Mind",
	"UFO Romance in the Night Sky",
	"Heian Alien",
	"Youkai Temple",
	"Returning Home from the Sky ~ Sky Dream",
};

// Touhou 12.5 Music
const std::string th125_musicNames[6] = {
	"Newshound",
	"The Mystery in Your Town",
	"Youkai Modern Colony",
	"Nemesis's Stronghold",
	"Bell of Avici ~ Infinite Nightmare",
	"Youkai Mountain ~ Mysterious Mountain",
};

// Touhou 12.5 Bosses and Spells
// Empty strings are for unused levels.
const std::string th125_bossAndSpells[140] = {
	// Level 1
	"Minoriko Aki",
	"Minoriko Aki",
	"Shizuha Aki - Autumn Sign \"Falling Blast\"",
	"Minoriko Aki - Fruit Sign \"Warm Colour Harvest\"",
	"Shizuha Aki - Wilted Path \"Lost Windrow\"",
	"Minoriko Aki - Baked Sweet Potato \"Sweet Potato Room\"",
	"",
	"",
	"",
	"",
	// Level 2
	"Parsee Mizuhashi",
	"Hina Kagiyama",
	"Parsee Mizuhashi - Jealousy \"Jealousy Bomber\"",
	"Hina Kagiyama - Cursed Field \"Purification River Deposit\"",
	"Parsee Mizuhashi - Resentment Art \"Grudge Returning\"",
	"Hina Kagiyama - Calamity \"Cursed Hina Dolls\"",
	"",
	"",
	"",
	"",
	// Level 3
	"Yamame Kurodani",
	"Kogasa Tatara - Umbrella Sign \"One-Legged Return Hit\"",
	"Kisume - Well Bucket \"Like a Moth to a Well\"",
	"Yamame Kurodani - Small Thread \"Kandata's Rope\"",
	"Kogasa Tatara - Rainbow Sign \"Over The Rainbow\"",
	"Kisume - Well Bucket \"Well Destructor\"",
	"Yamame Kurodani - Poison Sign \"Japanese Foliage Spider\"",
	"Kogasa Tatara - Umbrella Sign \"Flurry Travelers\"",
	"",
	"",
	// Level 4
	"Nitori Kawashiro",
	"Momiji Inubashiri",
	"Nitori Kawashiro - Water Sign \"Water Carpet\"",
	"Momiji Inubashiri - Dog Sign \"Rabies Bite\"",
	"Nitori Kawashiro - River Sign \"Dividing Edge\"",
	"Momiji Inubashiri - Mountain Nomad \"Expellee's Canaan\"",
	"Nitori Kawashiro - Kappa \"Dried Shirikodama\"",
	"",
	"",
	"",
	// Level 5
	"Ichirin Kumoi",
	"Minamitsu Murasa",
	"Ichirin Kumoi - Fist \"Falling Sky Hammer\"",
	"Minamitsu Murasa - Anchor Sign \"Phantom Ship's Long-Term Anchorage\"",
	"Ichirin Kumoi - Lightning \"Electrified Nyuudou\"",
	"Minamitsu Murasa - Flood \"Venus of the Bilge\"",
	"Ichirin Kumoi - Iron Fist \"Nyuudou Nyoki\"",
	"Minamitsu Murasa - \"Deep Sinker\"",
	"",
	"",
	// Level 6
	"Yuugi Hoshiguma",
	"Suika Ibuki",
	"Yuugi Hoshiguma - Light Oni \"Adamant Helix\"",
	"Suika Ibuki - Oni Sign \"Pea-Sized Needle Hell\"",
	"Yuugi Hoshiguma - Oni Sign \"Dreadful Raging Waves\"",
	"Suika Ibuki - Hell \"Purgatorial Sigh\"",
	"Yuugi Hoshiguma - Oni's Voice \"Annihilating Roar\"",
	"Suika Ibuki - Oni Sign \"Missing Power\"",
	"",
	"",
	// Level 7
	"Shou Toramaru",
	"Nazrin",
	"Shou Toramaru - Tiger Sign \"Hungry Tiger\"",
	"Nazrin - Rod Sign \"Nazrin Rod\"",
	"Shou Toramaru - Heaven Sign \"Scorched Earth Mandala\"",
	"Nazrin - Treasure \"Gold Rush\"",
	"Shou Toramaru - Treasure Sign \"Dazzling Gold\"",
	"",
	"",
	"",
	// Level 8
	"Rin Kaenbyou",
	"Utsuho Reiuji - Fusion \"Melting White\"",
	"Rin Kaenbyou - Death Sign \"Ghost Town\"",
	"Utsuho Reiuji - Giant Star \"Red Giant\"",
	"Rin Kaenbyou - \"Corpse Shopping District\"",
	"Utsuho Reiuji - Star Sign \"Fall of Great Stars\"",
	"Rin Kaenbyou - Drunkard's Walk \"Cat Random Walk\"",
	"Utsuho Reiuji - Seven Stars \"Septentrion\"",
	"",
	"",
	// Level 9
	"Satori Komeiji",
	"Koishi Komeiji - Heart Sign \"Selfless Love\"",
	"Satori Komeiji - Brain Sign \"Brain Fingerprint\"",
	"Koishi Komeiji - Memory \"DNA's Flaw\"",
	"Satori Komeiji - Heart Flower \"Camera - Shy Rose\"",
	"Koishi Komeiji - \"Embryo's Dream\"",
	"Satori Komeiji - Recollection \"Vague Recollection of Kinkaku-ji\"",
	"Koishi Komeiji - \"Rose Hell\"",
	"",
	"",
	// Level 10
	"Tenshi Hinanawi - Temperament \"High-Spirited Sword",
	"Iku Nagae - Thunder Sign \"Lightning Fish\"",
	"Tenshi Hinanawi - Earthquake \"Escarpment Evacuation Route\"",
	"Iku Nagae - Orb Sign \"Orb of the Five-Clawed Dragon\"",
	"Tenshi Hinanawi - Keystone \"Kaname Funnel\"",
	"Iku Nagae - Dragon Palace \"Dance of Sea Breams and Flounders\"",
	"Tenshi Hinanawi - \"Sky of Scarlet Perception of All Humankind\"",
	"Iku Nagae - Dragon Fish \"Oarfish's Swimming Shot\"",
	"",
	"",
	// Level 11
	"Kanako Yasaka",
	"Suwako Moriya - Divine Sakura \"Full Sakura Blizzard\"",
	"Kanako Yasaka - Snake Sign \"Ground Serpent\"",
	"Suwako Moriya - Hime River \"Princess Jade Green\"",
	"Kanako Yasaka - Divine Pillar \"Meteoric Onbashira\"",
	"Suwako Moriya - Iron Ring \"Mythical Ring\"",
	"Kanako Yasaka - Fleeting Path \"Omiwatari Cross\"",
	"Suwako Moriya - Native God \"Mishaguji-sama\"",
	"",
	"",
	// Level 12
	"Byakuren Hijiri",
	"Nue Houjuu - Unidentified \"Purple Mirror\"",
	"Byakuren Hijiri - \"Yuugyou Hijiri\"",
	"Nue Houjuu - Unidentified \"Red Cloak, Blue Cloak\"",
	"Byakuren Hijiri - Syncretism \"Manifestation of Mahavairocana\"",
	"Nue Houjuu - Unidentified \"Ms.Hanako of the Toilet\"",
	"Byakuren Hijiri - \"Star Sword Apologetics\"",
	"Nue Houjuu - \"Danmaku X from a Wandering Star\"",
	"",
	"",
	// Level EX
	"Reimu Hakurei - Talisman \"Exorcism of the Newspaper Subscription Solicitors\"",
	"Marisa Kirisame - Star Sign \"Oort Cloud\"",
	"Sanae Kochiya - Miracle \"Divine Wind of the Kouan Era\"",
	"Reimu Hakurei - Barrier \"Paparazzi-Repelling Barrier\"",
	"Marisa Kirisame - Celestial Apparatus \"Orreries Solar System\"",
	"Sanae Kochiya - Frog Sign \"Wily Toad\"",
	"Reimu Hakurei - Dream Sign \"Fantasy Dimensional Rift\"",
	"Marisa Kirisame - Comet \"Blazing Star\"",
	"Sanae Kochiya - Youkai Extermination \"Youkai Power Spoiler\"",
	"",
	// Level SP (??)
	"Hatate Himekaidou",
	"Hatate Himekaidou - Reporting \"Hatate Himekaidou's Reporting Training\"",
	"Hatate Himekaidou - Continuous Shooting \"Rapid Shot\"",
	"Hatate Himekaidou - Far-sightedness \"Tengu Psychography\"",
	"Aya Shameimaru",
	"Aya Shameimaru - Reporting \"Aya Shameimaru's Coercive Reporting\"",
	"Aya Shameimaru - Telescoping \"Candid Shot\"",
	"Aya Shameimaru - Snapshot \"Fast Shot\"",
	"Aya Shameimaru - \"Illusionary Dominance\"",
	"",
};

// Touhou 12.8
const std::string th12_8_musicNames[12] = {
	"The Refrain of the Lovely Great War",
	"Year-Round Absorbed Curiosity",
	"Staking Your Life on a Prank",
	"unused",
	"unused",
	"A Midnight Fairy Dance",
	"Great Fairy Wars ~ Fairy Wars",
	"An Ice Fairy in Spring",
	"Player's Score",
	"An Ice Fairy in Spring - still -",
	"Loose Rain",
	"Magus Night",
};

// Touhou 13
const std::string th13_spellCardName[127] = {
	// stage 1 (14 cards)
	"Symbol \"Dance of the Dead Butterflies\"",
	"Symbol \"Dance of the Dead Butterflies\"",
	"Symbol \"Dance of the Dead Butterflies - Cherry Blossoms -\"",
	"Symbol \"Dance of the Dead Butterflies - Cherry Blossoms -\"",
	"Ghostly Butterfly \"Ghost Spot\"",
	"Ghostly Butterfly \"Ghost Spot\"",
	"Ghostly Butterfly \"Ghost Spot - Cherry Blossoms -\"",
	"Ghostly Butterfly \"Ghost Spot - Cherry Blossoms -\"",
	"Nether Sign \"Endless Evening Cherry Blossoms\"",
	"Nether Sign \"Endless Evening Cherry Blossoms\"",
	"Nether Sign \"Endless Evening Cherry Blossoms\"",
	"Nether Sign \"Endless Evening Cherry Blossoms\"",
	"Cherry Blossom Sign \"Saigyou Cherry Blossom Blizzard\"",
	"Cherry Blossom Sign \"Saigyou Cherry Blossom Blizzard\"",
	// stage 2 (16 cards)
	"Echo Sign \"Mountain Echo\"",
	"Echo Sign \"Mountain Echo\"",
	"Echo Sign \"Mountain Echo Scramble\"",
	"Echo Sign \"Mountain Echo Scramble\"",
	"Echo Sign \"Power Resonance\"",
	"Echo Sign \"Power Resonance\"",
	"Echo Sign \"Power Resonance\"",
	"Echo Sign \"Power Resonance\"",
	"Mountain Echo \"Long-Range Echo\"",
	"Mountain Echo \"Long-Range Echo\"",
	"Mountain Echo \"Amplify Echo\"",
	"Mountain Echo \"Amplify Echo\"",
	"Great Voice \"Charged Cry\"",
	"Great Voice \"Charged Cry\"",
	"Great Voice \"Charged Yahoo!\"",
	"Great Voice \"Charged Yahoo!\"",
	// stage 3 (14 cards)
	"Rainbow Sign \"Umbrella Cyclone\"",
	"Rainbow Sign \"Umbrella Cyclone\"",
	"Recovery \"Heal By Desire\"",
	"Recovery \"Heal By Desire\"",
	"Recovery \"Heal By Desire\"",
	"Recovery \"Heal By Desire\"",
	"Poison Nail \"Poison Raze\"",
	"Poison Nail \"Poison Raze\"",
	"Poison Nail \"Poison Murder\"",
	"Poison Nail \"Poison Murder\"",
	"Desire Sign \"Score Desire Spirit Invitation\"",
	"Desire Sign \"Score Desire Spirit Invitation\"",
	"Desire Sign \"Score Desire Eater\"",
	"Desire Sign \"Score Desire Eater\"",
	// stage 4 (15 cards)
	"Evil Sign \"Yang Xiaogui\"",
	"Evil Sign \"Guhun Yegui\"",
	"Evil Sign \"Guhun Yegui\"",
	"Demonify \"Zouhuo Rumo\"",
	"Demonify \"Zouhuo Rumo\"",
	"Demonify \"Zouhuo Rumo\"",
	"Demonify \"Zouhuo Rumo\"",
	"Possession \"Corpse Tongji\"",
	"Possession \"Corpse Tongji\"",
	"Spirit Link \"Tongling Yoshika\"",
	"Spirit Link \"Tongling Yoshika\"",
	"Path Sign \"Tao Fetal Movement\"",
	"Path Sign \"Tao Fetal Movement\"",
	"Path Sign \"Tao Fetal Movement\"",
	"Path Sign \"Tao Fetal Movement\"",
	// stage 5 (19 cards)
	"Thunder Arrow \"Gagouji's Cyclone\"",
	"Thunder Arrow \"Gagouji's Cyclone\"",
	"Thunder Arrow \"Gagouji's Tornado\"",
	"Heaven Sign \"Rainy Iwafune\"",
	"Heaven Sign \"Rainy Iwafune\"",
	"Heaven Sign \"Iwafune Ascending to Heaven\"",
	"Heaven Sign \"Iwafune Ascending to Heaven\"",
	"Throwing Dishes \"Mononobe's Eighty Sake Cups\"",
	"Throwing Dishes \"Mononobe's Eighty Sake Cups\"",
	"Throwing Dishes \"Mononobe's Eighty Sake Cups\"",
	"Throwing Dishes \"Mononobe's Eighty Sake Cups\"",
	"Blaze Sign \"Blazing Winds of Haibutsu\"",
	"Blaze Sign \"Blazing Winds of Haibutsu\"",
	"Blaze Sign \"Sakuraiji in Flames\"",
	"Blaze Sign \"Sakuraiji in Flames\"",
	"Saint Girl \"Oomonoimi's Dinner\"",
	"Saint Girl \"Oomonoimi's Dinner\"",
	"Saint Girl \"Oomonoimi's Dinner\"",
	"Saint Girl \"Oomonoimi's Dinner\"",
	// stage 6 (28 cards)
	"Honor \"Colors of Twelve Levels\"",
	"Honor \"Colors of Twelve Levels\"",
	"Honor \"Ranks of Twelve Levels\"",
	"Honor \"Ranks of Twelve Levels\"",
	"Hermit Sign \"Taoist of the Land of the Rising Sun\"",
	"Hermit Sign \"Taoist of the Land of the Rising Sun\"",
	"Hermit Sign \"Emperor of the Land of the Rising Sun\"",
	"Hermit Sign \"Emperor of the Land of the Rising Sun\"",
	"Summon \"Royal Clan's Chaotic Dance\"",
	"Summon \"Royal Clan's Chaotic Dance\"",
	"Summon \"Royal Clan's Chaotic Dance\"",
	"Summon \"Royal Clan's Chaotic Dance\"",
	"Secret Treasure \"Armillary Sphere of Ikaruga-dera\"",
	"Secret Treasure \"Armillary Sphere of Ikaruga-dera\"",
	"Secret Treasure \"Armillary Sphere of Ikaruga-dera\"",
	"Secret Treasure \"Prince Shotoku's Out-of-Place Artifact\"",
	"Light Sign \"Halo of the Guse Kannon\"",
	"Light Sign \"Halo of the Guse Kannon\"",
	"Light Sign \"Guse Flash\"",
	"Light Sign \"Guse Flash\"",
	"Discernment \"Laser of Seventeen Articles\"",
	"Discernment \"Laser of Seventeen Articles\"",
	"Divine Light \"Honour the Avoidance of Defiance\"",
	"Divine Light \"Honour the Avoidance of Defiance\"",
	"\"Falling Stars on Divine Spirit Mausoleum\"",
	"\"Falling Stars on Divine Spirit Mausoleum\"",
	"\"Newborn Divine Spirit\"",
	"\"Newborn Divine Spirit\"",
	// extra stage (13 cards)
	"Unknown \"Will-o'-wisps in Unknown Orbit\"",
	"Unknown \"Skyfish with Unknown Shape\"",
	"Unknown \"Youkai Orb of Unknown Mechanics\"",
	"First Duel \"Primate Danmaku Transformation\"",
	"Second Duel \"Carnivorous Danmaku Transformation\"",
	"Third Duel \"Avian Danmaku Transformation\"",
	"Fourth Duel \"Amphibian Danmaku Transformation\"",
	"Fifth Duel \"Scrolls of Frolicking Animals\"",
	"Sixth Duel \"Tanuki's Monstrous School\"",
	"Seventh Duel \"Wild Deserted Island\"",
	"Transformation \"Pseudo-Exorcism of the Stupid Shrine Maiden\"",
	"\"Mamizou Danmaku in Ten Transformations\"",
	"Mujina Sign \"Full Moon Pompokolin\"",
	// OVERDRIVE (8 cards)
	"Cherry Blossom Sign \"Cherry Blossom Blizzard Hell\"",
	"Mountain Echo \"Yamabiko's Specialty Echo Demonstration\"",
	"Poison Nail \"Undead Murderer\"",
	"Path Sign \"TAO Fetal Movement ~Dao~\"",
	"Vengeful Spirit \"Iruka's Thunder\"",
	"Saint Girl \"Sun Goddess's Sacrifice\"",
	"\"Divine Spirits' Macrocosm\"",
	"\"Wild Carpet\""
};

// Touhou 13 Music
const std::string th13_musicNames[22] = {
	"Spirit of Avarice",
	"Night Sakura of Dead Spirits",
	"unused",
	"Ghost Lead",
	"Welcome to Youkai Temple",
	"unused",
	"Youkai Girl at the Gate",
	"Let's Live in a Lovely Cemetery",
	"Rigid Paradise",
	"unused",
	"Desire Drive",
	"Old Yuanxian",
	"The Hall of Dreams' Great Mausoleum",
	"unused",
	"unused",
	"Omiwa Legend",
	"Starry Sky of Small Desires",
	"Shoutoku Legend ~ True Administrator",
	"A New Wind at the Shrine",
	"Desire Dream",
	"Youkai Back Shrine Road",
	"Futatsuiwa from Sado",
};

// Touhou 14 Spell Cards
const std::string th14_spellCardName[120] = {
	// stage 1 (1-10) (10 cards)
	"Ice Sign \"Ultimate Blizzard\"",
	"Ice Sign \"Ultimate Blizzard\"",
	"Water Sign \"Tail Fin Slap\"",
	"Water Sign \"Tail Fin Slap\"",
	"Water Sign \"Tail Fin Slap\"",
	"Water Sign \"Tail Fin Slap\"",
	"Scale Sign \"Scale Wave\"",
	"Scale Sign \"Scale Wave\"",
	"Scale Sign \"Raging Waves of the Reversed Scale\"",
	"Scale Sign \"Great Raging Waves of the Reversed Scale\"",
	// stage 2 (11-26) (16 cards)
	"Flight Sign \"Flying Head\"",
	"Flight Sign \"Flying Head\"",
	"Flight Sign \"Flying Head\"",
	"Flight Sign \"Flying Head\"",
	"Neck Sign \"Close - Eye Shot\"",
	"Neck Sign \"Close - Eye Shot\"",
	"Neck Sign \"Rokurokubi Flight\"",
	"Neck Sign \"Rokurokubi Flight\"",
	"Flying Head \"Multiplicative Head\"",
	"Flying Head \"Multiplicative Head\"",
	"Flying Head \"Seventh Head\"",
	"Flying Head \"Ninth Head\"",
	"Flying Head \"Dullahan Night\"",
	"Flying Head \"Dullahan Night\"",
	"Flying Head \"Dullahan Night\"",
	"Flying Head \"Dullahan Night\"",
	// stage 3 (27-40) (14 cards)
	"Fang Sign \"Moonlit Canine Teeth\"",
	"Fang Sign \"Moonlit Canine Teeth\"",
	"Transformation \"Triangle Fang\"",
	"Transformation \"Triangle Fang\"",
	"Transformation \"Star Fang\"",
	"Transformation \"Star Fang\"",
	"Roar \"Strange Roar\"",
	"Roar \"Strange Roar\"",
	"Roar \"Full Moon Howling\"",
	"Roar \"Full Moon Howling\"",
	"Wolf Sign \"Star Ring Pounce\"",
	"Wolf Sign \"Star Ring Pounce\"",
	"Sirius \"High-Speed Pounce\"",
	"Sirius \"High-Speed Pounce\"",
	// stage 4 (41-64) (24 cards)
	"Heikyoku \"Sounds of Jetavana's Bell\"",
	"Heikyoku \"Sounds of Jetavana's Bell\"",
	"Heikyoku \"Sounds of Jetavana's Bell\"",
	"Heikyoku \"Sounds of Jetavana's Bell\"",
	"Vengeful Spirit \"Hoichi the Earless\"",
	"Vengeful Spirit \"Hoichi the Earless\"",
	"Vengeful Spirit \"Great Vengeful Spirit of Taira\"",
	"Vengeful Spirit \"Great Vengeful Spirit of Taira\"",
	"Music Sign \"Wicked Musical Score\"",
	"Music Sign \"Wicked Musical Score\"",
	"Music Sign \"Malicious Musical Score\"",
	"Music Sign \"Double Score\"",
	"Koto Sign \"Sounds of Anicca's Koto\"",
	"Koto Sign \"Sounds of Anicca's Koto\"",
	"Koto Sign \"Sounds of Anicca's Koto\"",
	"Koto Sign \"Sounds of Anicca's Koto\"",
	"Echo Sign \"Heian's Reverberation\"",
	"Echo Sign \"Heian's Reverberation\"",
	"Echo Sign \"Echo Chamber\"",
	"Echo Sign \"Echo Chamber\"",
	"Koto Music \"Social Upheaval Koto Dirge\"",
	"Koto Music \"Social Upheaval Koto Dirge\"",
	"Koto Music \"Social Upheaval Requiem\"",
	"Koto Music \"Social Upheaval Requiem\"",
	// stage 5 (65-84) (20 cards)
	"Deceit Sign \"Reverse Needle Attack\"",
	"Deceit Sign \"Reverse Needle Attack\"",
	"Deceit Sign \"Reverse Needle Attack\"",
	"Deceit Sign \"Reverse Needle Attack\"",
	"Reverse Sign \"Danmaku Through the Looking-Glass\"",
	"Reverse Sign \"Danmaku Through the Looking-Glass\"",
	"Reverse Sign \"Evil in the Mirror\"",
	"Reverse Sign \"Evil in the Mirror\"",
	"Reverse Sign \"This Side Down\"",
	"Reverse Sign \"This Side Down\"",
	"Reverse Sign \"Overturning All Under Heaven\"",
	"Reverse Sign \"Overturning All Under Heaven\"",
	"Reverse Bow \"Dream Bow of Heaven & Earth\"",
	"Reverse Bow \"Dream Bow of Heaven & Earth\"",
	"Reverse Bow \"Decree of the Dream Bow of Heaven & Earth\"",
	"Reverse Bow \"Decree of the Dream Bow of Heaven & Earth\"",
	"Turnabout \"Reverse Hierarchy\"",
	"Turnabout \"Reverse Hierarchy\"",
	"Turnabout \"Change Air Brave\"",
	"Turnabout \"Change Air Brave\"",
	// stage 6 (85-108) (24 cards)
	"Small Bullet \"Inchling's Path\"",
	"Small Bullet \"Inchling's Path\"",
	"Small Bullet \"Inchling's Thorny Path\"",
	"Small Bullet \"Inchling's Thorny Path\"",
	"Mallet \"Grow Bigger!\"",
	"Mallet \"Grow Bigger!\"",
	"Mallet \"Grow Even Bigger!\"",
	"Mallet \"Grow Even Bigger!\"",
	"Bewitched Sword \"Shining Needle Sword\"",
	"Bewitched Sword \"Shining Needle Sword\"",
	"Bewitched Sword \"Shining Needle Sword\"",
	"Bewitched Sword \"Shining Needle Sword\"",
	"Mallet \"You Grow Bigger!\"",
	"Mallet \"You Grow Bigger!\"",
	"Mallet \"You Grow Bigger!\"",
	"Mallet \"You Grow Bigger!\"",
	"\"Attack on Dwarf\"",
	"\"Attack on Dwarf\"",
	"\"Wall of Issun\"",
	"\"Wall of Issun\"",
	"\"Hop-o'-My-Thumb Seven\"",
	"\"Hop-o'-My-Thumb Seven\"",
	"\"The Seven Issun-Boushi\"",
	"\"The Seven Issun-Boushi\"",
	// extra stage (109-120) (12 cards)
	"String Music \"Storm Ensemble\"",
	"String Music \"Joururi World\"",
	"First Drum \"Raging Temple Taiko\"",
	"Second Drum \"Vengeful Spirit Aya-no-Tsuzumi\"",
	"Third Drum \"Three Strikes at Midnight\"",
	"Death Drum \"Land Percuss\"",
	"Fifth Drum \"Den-Den Daiko\"",
	"Sixth Drum \"Alternate Sticking\"",
	"Seventh Drum \"High Speed Taiko Rocket\"",
	"Eighth Drum \"Thunder God's Anger\"",
	"\"Blue Lady Show\"",
	"\"Pristine Beat\"",
};

// Touhou 14 Music
const std::string th14_musicNames[20] = {
	"unused",
	"Mysterious Purification Rod",
	"Mist Lake",
	"Mermaid from the Uncharted Land",
	"Humans and Youkai Traversing the Canal",
	"Dullahan Under the Willows",
	"Bamboo Forest of the Full Moon",
	"Lonesome Werewolf",
	"unused",
	"Magical Storm",
	"Illusionary Joururi",
	"The Shining Needle Castle Sinking in the Air",
	"Reverse Ideology",
	"unused",
	"The Exaggerated Castle Keep",
	"Inchlings of the Shining Needle ~ Little Princess",
	"Magical Power of the Mallet",
	"Mysterious, Mysterious Tools",
	"Thunderclouds of Magical Power",
	"Primordial Beat ~ Pristine Beat",
};

// Touhou 14.3 stage names
const std::string th143_stageNames[76] = {
	"unused",
	// Day 1
	"Cheat Against Impossible Danmaku",
	"Unavoidable Tailfin",
	"Violent Freeze",
	"Encroaching Terror",
	"Gee Cirno, You Sure Are Conceited",
	"Persistent School of Fish",
	// Day 2
	"Youkai at the Gate, Reading An Unlearnable Sutra",
	"Longest-Necked Kayan in the World",
	"An Ear-Piercing Scream",
	"Do Beams or Romance Come From Her Eyes?",
	"Neverending Sutra Book",
	"Two-Headed Dragon",
	// Day 3
	"Multiplying Pursuers",
	"Dangers Abound on a Full Moon",
	"A Tranquil Dystopia",
	"Oh, If Only the Screen Had No Edges",
	"Do I Smell?",
	"The Array of Edo's Fight",
	"Even Immortals Get Tired",
	// Day 4
	"What Appears at the Graveyard",
	"A Rather Suspicious Vitality",
	"Evaporating Butterflies",
	"Get Hit, and You're Zombified",
	"Walls Closing In",
	"Will you Dodge it, Erase it, or...?",
	"Enemies have Freedom Too",
	// Day 5
	"Can't Just Rely On Items Alone",
	"Stop Aiming At Me!",
	"Intertwined Melody",
	"If I Could Just Break Those Stupid Drums...",
	"Requiem",
	"Silent Score",
	"Super-Heavy Bass",
	"Must we Avoid this Beautiful Night?",
	// Day 6
	"The Big includes the Small",
	"The Camera is Mightier than the Pen",
	"The Latest Cellphone Camera",
	"Unreliable Waterfalls",
	"Bite a Lot to Train Your Jaw",
	"Drowning Gun With a Shady Story",
	"An Overblown Article Written by a Shut-In",
	"Don't Give Her a Photo Op",
	// Day 7
	"The Big includes the Small",
	"I Don't Know What Happened",
	"Cut the Flesh, Sever the Bone",
	"Systematic Reptile",
	"Firing Indiscriminately",
	"Don't You Think That's Devious?",
	"She's Also Firing Indiscriminately",
	"There's No Venom in a Common Snake",
	// Day 8
	"Turn Everyone into an Enemy!",
	"The Perfect Entrapment",
	"You Can't Rely On Items Alone",
	"Teeeensy Danmaku",
	"Normal Shots Are Scary, Huh?",
	"They're Nice, Compared to the Others",
	"Amanojaku Aren't Necessarily Oni",
	// Day 9
	"Yaaay, Rising!",
	"Itoigawa is Known for Its Jade",
	"Sometimes a Boatman Must Climb a Mountain",
	"So the Tiny Ones Are Real Too?",
	"One Should Fear Angering the Gods",
	"My Precious Amphibians...!",
	"Double Dragon",
	"Desperate to Gather Rumors",
	// Day 10
	"The Fourth Monster appears",
	"Did You Plan on Becoming God?",
	"So You Got Youkai Piled Up, Too",
	"12-Second Nightmare",
	"Cheating Back at You...!",
	"Round 'n' Round 'n' Round",
	"Teacher, I Don't Want to Study History Anymore",
	"Pickling Stone Drill",
	"I Got You... Huh!?",
	"You Just Have to Persevere",
};

// Touhou 14.3 Bosses and Spells
const std::string th143_bossAndSpells[76] = {
	"unused",
	// Day 1
	"Yatsuhashi Tsukumo",
	"Wakasagihime - Water Sign \"Lunatic Red Slap\"",
	"Cirno - Ice Sign \"Perfect Glacialist\"",
	"Wakasagihime - Tide Sign \"Tidal Wave of the Lake\"",
	"Cirno - Ice King \"Frost King\"",
	"Wakasagihime - Fish Sign \"School of Fish\"",
	// Day 2
	"Kyouko Kasodani - Scream \"Primal Scream\"",
	"Sekibanki - Flying Neck \"Extreme Long Neck\"",
	"Kyouko Kasodani - Piercing Sound \"Piercing Circle\"",
	"Sekibanki - Glinting Eyes \"Hell's Ray\"",
	"Kyouko Kasodani - Sutra \"Infinite Nenbutsu\"",
	"Sekibanki - Flying Neck \"Twin Rokuro Head\"",
	// Day 3
	"Kagerou Imaizumi",
	"Kagerou Imaizumi - Full Moon \"Full Moon Roar\"",
	"Keine Kamishirasawa - \"20XX: An Afterlife Odyssey\"",
	"Fujiwara no Mokou - Regretful Life \"Immortality's Reckless Sacrifice\"",
	"Kagerou Imaizumi - Wolf Fang \"Bloodthirsty Wolf Fang\"",
	"Keine Kamishirasawa - Great Fire \"Flower of Edo\"",
	"Fujiwara no Mokou - \"Fire Bird -Legend of Immortality-\" || \"Resurrection\"",
	// Day 4
	"Yuyuko Saigyouji",
	"Seiga Kaku - Demonify \"Excessive Zouhuo Rumo\"",
	"Yuyuko Saigyouji - Butterfly Sign \"Flower, Butterfly, Wind, and Moon\"",
	"Yoshika Miyako - Poison Nail \"Zombie Claw\"",
	"Seiga Kaku - Hermit Arts \"Wall Runner\"",
	"Yuyuko Saigyouji - Cherry Blossom \"Lovely Cherry Blossom Blizzard\"",
	"Seiga Kaku - Hermit Arts \"Wall - Phasing Wormhole\"",
	// Day 5
	"Raiko Horikawa",
	"Yatsuhashi Tsukumo - Koto Sign \"Ame no Norigoto\"",
	"Benben Tsukumo - Noise Sign \"Biwa of Euphoric Song\"",
	"Raiko Horikawa - Thunder Sign \"Den-Den Daiko of Rage\"",
	"Yatsuhashi Tsukumo - Elegy \"Human & Koto Die Together\"",
	"Benben Tsukumo - Score \"Score Web\"",
	"Raiko Horikawa - Taiko \"Fantastic Woofer\"",
	"Benben and Yatsuhashi Tsukumo - Double Chant \"Song of Falling Stars\"",
	// Day 6
	"Mamizou Futatsuiwa",
	"Aya Shameimaru - Photography \"Quick-Shooting Tengu Scoop\"",
	"Hatate Himekaidou - Photography \"Full Panoramic Shot\"",
	"Nitori Kawashiro - Waterfall Sign \"Shiraito Falls\"",
	"Momiji Inubashiri - Fang Sign \"Chewing Satisfaction\"",
	"Nitori Kawashiro - Waterfall Sign \"Kegon Gun\"",
	"Hatate Himekaidou - Photography \"Secluded Paparazzi\"",
	"Aya Shameimaru - \"Instant Shot Journalist\"",
	// Day 7
	"Marisa Kirisame - Love Sign \"Wide Master\"",
	"Sakuya Izayoi - Time Sign \"Time Stopper Sakuya\"",
	"Youmu Konpaku - Light Sign \"Light Flash of the Netherworld\"",
	"Sanae Kochiya - Snake Sign \"Bind Snake Come On\"",
	"Marisa Kirisame - Love Sign \"Machine Gun Spark\"",
	"Sakuya Izayoi - Time Sign \"Changeling Magic\"",
	"Youmu Konpaku - Higan Sword \"Hacking Slashes of Hell & Paradise\"",
	"Sanae Kochiya - Snake Sign \"Green Snake Come On\"",
	// Day 8
	"Shinmyoumaru Sukuna",
	"Reimu Hakurei - Mikuji \"Rule Violation Barrier\"",
	"Mamizou Futatsuiwa - \"If the Cuckoo Does Not Sing, Wait For It to Cry\"",
	"Shinmyoumaru Sukuna - \"Inchlings' Hell\"",
	"Reimu Hakurei - \"Persuasion Needle\"",
	"Mamizou Futatsuiwa - \"Humans Are Nice!\"",
	"Shinmyoumaru Sukuna - Shining Needle \"Oni-Slaying, Eye-Stabbing Needle\"",
	// Day 9
	"Kanako Yasaka - Onbashira \"Rising Onbashira\"",
	"Suwako Moriya - Green Stone \"Jade Break\"",
	"Mononobe no Futo - Old Boat \"Ancient Ship\"",
	"Suika Ibuki - Oni Crowd \"Imp Swarm\"",
	"Kanako Yasaka - \"Sacred Authority of the Gods\"",
	"Suwako Moriya - Frog Sign \"Bloody Mound of Red Frogs\"",
	"Mononobe no Futo - Heat Dragon \"Blazing Dragon Veins\"",
	"Suika Ibuki - Oni Crowd \"Hundred Oni Kaburo\"",
	// Day 10
	"Byakuren Hijiri - \"Binding Laws of Hari\"",
	"Toyosatomimi no Miko - \"My Way is Truly That of Heaven!\"",
	"Tenshi Hinanawi - \"Sky of Scarlet Perception of All Youkaikind\"",
	"Remilia Scarlet - \"Fitful Nightmare\"",
	"Yukari Yakumo - \"Impossible Danmaku Barrier\"",
	"Byakuren Hijiri - \"Eyes of Brahma\"",
	"Toyosatomimi no Miko - \"Seventeen-Article Constitution Bombs\"",
	"Tenshi Hinanawi - \"Kashima Protection\"",
	"Remilia Scarlet - \"Duck, Duck, Bat!\"",
	"Yukari Yakumo - \"Casebook of Luck, Resilience, and Perseverance\"",
};

// Touhou 14.3 Music
const std::string th143_musicNames[9] = {
	"Raise the Signal Fire of Cheating",
	"Cheat Against the Impossible Danmaku",
	"Midnight Spell Card",
	"Romantic Escape Flight",
	"Eternal Transient Reign",
	"Mermaid from the Uncharted Land",
	"Reverse Ideology",
	"Illusionary Joururi",
	"Youkai Mountain ~ Mysterious Mountain",
};

// Touhou 15 Music
const std::string th15_musicNames[18] = {
	"unused",
	"The Space Shrine Maiden Appears",
	"Unforgettable, the Nostalgic Greenery",
	"The Lake Reflects the Cleansed Moonlight",
	"The Rabbit Has Landed",
	"September Pumpkin",
	"The Mysterious Shrine Maiden Flying Through Space",
	"Eternal Spring Dream",
	"The Frozen Eternal Capital",
	"Reversed Wheel of Fortune",
	"Faraway Voyage of 380,000 Kilometers",
	"Pierrot of the Star-Spangled Banner",
	"The Sea that Reflects One's Home Planet",
	"Pure Furies ~ Whereabouts of the Heart",
	"The Moon as Seen from the Shrine",
	"The Space Shrine Maiden Returns Home",
	"A World of Nightmares Never Seen Before",
	"Pandemonic Planet",
};

// Touhou 16
const std::string th16_spellCardName[119] = {
	// stage 1 (8 cards)
	"Butterfly Sign \"Minute Scales\"",
	"Butterfly Sign \"Minute Scales\"",
	"Butterfly Sign \"Swallowtail's Scales\"",
	"Butterfly Sign \"Swallowtail's Scales\"",
	"Butterfly Sign \"Fluttering Summer\"",
	"Butterfly Sign \"Fluttering Summer\"",
	"Butterfly Sign \"Midsummer Wingbeats\"",
	"Butterfly Sign \"Midsummer Wingbeats\"",
	// stage 2 (12 cards)
	"Rain Sign \"Imprisoning Autumn Rain\"",
	"Rain Sign \"Imprisoning Autumn Rain\"",
	"Rain Sign \"Cursed Torrential Rain\"",
	"Rain Sign \"Cursed Torrential Rain\"",
	"Blade Sign \"Yamanba's Kitchen Knife Sharpening\"",
	"Blade Sign \"Yamanba's Kitchen Knife Sharpening\"",
	"Blade Sign \"Yamanba's Oni Kitchen Knife Sharpening\"",
	"Blade Sign \"Yamanba's Oni Kitchen Knife Sharpening\"",
	"Blade Exhaustion Sign \"Mountain Murder\"",
	"Blade Exhaustion Sign \"Mountain Murder\"",
	"Blade Exhaustion Sign \"Bloody Mountain Murder\"",
	"Blade Exhaustion Sign \"Bloody Mountain Murder\"",
	// stage 3 (14 cards)
	"Spring Sign \"Surprise Spring\"",
	"Spring Sign \"Surprise Spring\"",
	"Dog Sign \"Stray Dog's Walk\"",
	"Dog Sign \"Stray Dog's Walk\"",
	"Hound Sign \"Mountain Hound's Walk\"",
	"Hound Sign \"Mountain Hound's Walk\"",
	"Spinning Top \"Koma-Inu Spin\"",
	"Spinning Top \"Koma-Inu Spin\"",
	"Spinning Top \"Koma-Inu Spin\"",
	"Spinning Top \"Curl Up And Die\"",
	"Koma Sign \"Solitary A-Un Breathing\"",
	"Koma Sign \"Solitary A-Un Breathing\"",
	"Koma Sign \"Solitary A-Un Breathing\"",
	"Koma Sign \"Solitary A-Un Breathing\"",
	// stage 4 (12 cards)
	"Magic Sign \"Instant Bodhi\"",
	"Magic Sign \"Instant Bodhi\"",
	"Magic Sign \"Bodhi in an Instant\"",
	"Magic Sign \"Bodhi in an Instant\"",
	"Magic Sign \"Bullet Golem\"",
	"Magic Sign \"Bullet Golem\"",
	"Magic Sign \"Gigantic Pet Bullet Lifeform\"",
	"Magic Sign \"Gigantic Pet Bullet Lifeform\"",
	"Jizo \"Criminal Salvation\"",
	"Jizo \"Criminal Salvation\"",
	"Jizo \"Hellfire Salvation\"",
	"Jizo \"Hellfire Salvation\"",
	// stage 5 (24 cards)
	"Bamboo Sign \"Bamboo Spear Dance\"",
	"Bamboo Sign \"Bamboo Spear Dance\"",
	"Bamboo Sign \"Bamboo Crazy Dance\"",
	"Bamboo Sign \"Bamboo Crazy Dance\"",
	"Myoga Ginger \"Forget Your Name\"",
	"Myoga Ginger \"Forget Your Name\"",
	"Myoga Ginger \"Forget Your Name\"",
	"Myoga Ginger \"Forget Your Name\"",
	"Broadleaf Sign \"Tanabata Star Festival\"",
	"Broadleaf Sign \"Tanabata Star Festival\"",
	"Broadleaf Sign \"Tanabata Star Festival\"",
	"Broadleaf Sign \"Tanabata Star Festival\"",
	"Myoga Blessing \"Behind You\"",
	"Myoga Blessing \"Behind You\"",
	"Myoga Blessing \"Behind You\"",
	"Myoga Blessing \"Behind You\"",
	"Dance Sign \"Behind Festival\"",
	"Dance Sign \"Behind Festival\"",
	"Dance Sign \"Behind Festival\"",
	"Dance Sign \"Behind Festival\"",
	"Mad Dance \"Tengu-Odoshi\"",
	"Mad Dance \"Tengu-Odoshi\"",
	"Mad Dance \"Frenzied Tengu-Frightening\"",
	"Mad Dance \"Frenzied Tengu-Frightening\"",
	// stage 6 (36 cards, 16 of which are season finals damn I hate it)
	"Back Sign \"Halo of the Secret God\"",
	"Back Sign \"Halo of the Secret God\"",
	"Back Sign \"Halo of the Secret God\"",
	"Back Sign \"Halo of the Absolute Secret God\"",
	"Hidden Summer \"Scorch by Hot Summer\"",
	"Hidden Summer \"Scorch by Hot Summer\"",
	"Hidden Summer \"Scorched Earth of Abnormal Intense Heat\"",
	"Hidden Summer \"Scorched Earth of Abnormal Intense Heat\"",
	"Hidden Fall \"Die of Famine\"",
	"Hidden Fall \"Die of Famine\"",
	"Hidden Fall \"Preta of Abnormal Blight\"",
	"Hidden Fall \"Preta of Abnormal Blight\"",
	"Hidden Winter \"Black Snowman\"",
	"Hidden Winter \"Black Snowman\"",
	"Hidden Winter \"Snowman of Abnormal Snowfall\"",
	"Hidden Winter \"Snowman of Abnormal Snowfall\"",
	"Hidden Spring \"April Wizard\"",
	"Hidden Spring \"April Wizard\"",
	"Hidden Spring \"Black Magician of Abnormal Falling Petals\"",
	"Hidden Spring \"Black Magician of Abnormal Falling Petals\"",
	"\"Hidden Breezy Cherry Blossom\"",
	"\"Hidden Breezy Cherry Blossom\"",
	"\"Hidden Breezy Cherry Blossom\"",
	"\"Hidden Breezy Cherry Blossom\"",
	"\"Hidden Perfect Summer Ice\"",
	"\"Hidden Perfect Summer Ice\"",
	"\"Hidden Perfect Summer Ice\"",
	"\"Hidden Perfect Summer Ice\"",
	"\"Hidden Crazy Fall Wind\"",
	"\"Hidden Crazy Fall Wind\"",
	"\"Hidden Crazy Fall Wind\"",
	"\"Hidden Crazy Fall Wind\"",
	"\"Hidden Extreme Winter\"",
	"\"Hidden Extreme Winter\"",
	"\"Hidden Extreme Winter\"",
	"\"Hidden Extreme Winter\"",
	// extra stage (13 cards)
	"Drum Dance \"Powerful Cheers\"",
	"Mad Dance \"Crazy Backup Dance\"",
	"Bullet Dance \"Two-Eyed Typhoon\"",
	"Secret Ceremony \"Reverse Invoker\"",
	"Secret Ceremony \"Rear Shots of Betrayal\"",
	"Secret Ceremony \"Danmaku Dupion\"",
	"Secret Ceremony \"Fire of Ena\"",
	"Secret Ceremony \"Kyogen of the Back Door\"",
	"Secret Ceremony \"Matarah Dukkha\"",
	"Secret Ceremony \"Sword of the Seven Stars\"",
	"Secret Ceremony \"Performers Unattached to Society\"",
	"\"The Back Face's Dark Sarugaku\"",
	"\"Anarchy Bullet Hell\""
};

// Touhou 16 Music
const std::string th16_musicNames[18] = {
	"unused",
	"The Sky Where Cherry Blossoms Flutter Down",
	"A Star of Hope Rises in the Blue Sky",
	"A Midsummer Fairy's Dream",
	"The Colorless Wind on Youkai Mountain",
	"Deep-Mountain Encounter",
	"Swim in a Cherry Blossom-Colored Sea",
	"A Pair of Divine Beasts",
	"Illusionary White Traveler",
	"The Magic Straw-Hat Jizo",
	"Does the Forbidden Door Lead to This World, or the World Beyond?",
	"Crazy Backup Dancers",
	"The Concealed Four Seasons",
	"Into Backdoor",
	"Unnatural Nature",
	"White Traveler",
	"No More Going Through Doors",
	"Secret God Matara ~ Hidden Star in All Seasons.",
};

// Touhou 17
const std::string th17_spellCardName[101] = {
	// stage 1 (1-8, 8 cards)
	"Stone Sign \"Stone Woods\"",
	"Stone Sign \"Stone Woods\"",
	"Stone Sign \"Stone Conifers\"",
	"Stone Sign \"Stone Conifers\"",
	"Stone Sign \"Children's Limbo\"",
	"Stone Sign \"Children's Limbo\"",
	"Stone Sign \"Adult Children's Limbo\"",
	"Stone Sign \"Adult Children's Limbo\"",
	// stage 2 (9-20, 12 cards)
	"Stone Sign \"Stone Baby\"",
	"Stone Sign \"Stone Baby\"",
	"Stone Sign \"Heavy Stone Baby\"",
	"Stone Sign \"Heavy Stone Baby\"",
	"Drowning Sign \"Drowning in the Sanzu\"",
	"Drowning Sign \"Drowning in the Sanzu\"",
	"Drowning Sign \"Drowning in the Sanzu\"",
	"Drowning Sign \"Drowning in the Sanzu\"",
	"Oni Sign \"Demon Siege\"",
	"Oni Sign \"Demon Siege\"",
	"Oni Sign \"Hungry Demon Siege\"",
	"Oni Sign \"Hungry Demon Siege\"",
	// stage 3 (21-32, 12 cards)
	"Water Sign \"Ordeal of Water Distribution\"",
	"Water Sign \"Ordeal of Water Distribution\"",
	"Water Sign \"High-Level Purgatordeal of Water Distribution\"",
	"Water Sign \"Ultimate Purgatordeal of Water Distribution\"",
	"Light Sign \"Ordeal of Surveying\"",
	"Light Sign \"Ordeal of Surveying\"",
	"Light Sign \"High-Level Purgatordeal of Surveying\"",
	"Light Sign \"Ultimate Purgatordeal of Surveying\"",
	"Oni Sign \"Purgatordeal of Oniwatari\"",
	"Oni Sign \"Purgatordeal of Oniwatari\"",
	"Oni Sign \"High-Level Purgatordeal of Oniwatari\"",
	"Oni Sign \"Hell-Level Purgatordeal of Oniwatari\"",
	// stage 4 (33-44, 12 cards)
	"Tortoise Sign \"Tortoiseshell Hell\"",
	"Tortoise Sign \"Tortoiseshell Hell\"",
	"Tortoise Sign \"Tortoiseshell Hell\"",
	"Tortoise Sign \"Tortoiseshell Hell\"",
	"Oni Sign \"Beasts Attacking from the Rear\"",
	"Oni Sign \"Beasts Attacking from the Rear\"",
	"Oni Sign \"Mangy Beasts Attacking from the Rear\"",
	"Oni Sign \"Devilish Beasts Attacking from the Rear\"",
	"Dragon Sign \"Dragon Crest Bullets\"",
	"Dragon Sign \"Dragon Crest Bullets\"",
	"Dragon Sign \"Dragon Crest Bullets\"",
	"Dragon Sign \"Dragon Crest Bullets\"",
	// stage 5 (45-60, 16 cards)
	"Haniwa \"Archer Haniwa\"",
	"Haniwa \"Archer Haniwa\"",
	"Haniwa \"Skilled Archer Haniwa\"",
	"Haniwa \"Skilled Archer Haniwa\"",
	"Haniwa \"Fencer Haniwa\"",
	"Haniwa \"Fencer Haniwa\"",
	"Haniwa \"Skilled Fencer Haniwa\"",
	"Haniwa \"Skilled Fencer Haniwa\"",
	"Haniwa \"Cavalry Haniwa\"",
	"Haniwa \"Cavalry Haniwa\"",
	"Haniwa \"Skilled Cavalry Haniwa\"",
	"Haniwa \"Skilled Cavalry Haniwa\"",
	"Haniwa \"Hollow Inexhaustible Troops\"",
	"Haniwa \"Hollow Inexhaustible Troops\"",
	"Haniwa \"Undefeated Inexhaustible Troops\"",
	"Haniwa \"Undefeated Inexhaustible Troops\"",
	// stage 6 (61-88, 28 cards)
	"Square Shape \"Square-Shaped Sculpting Art\"",
	"Square Shape \"Square-Shaped Sculpting Art\"",
	"Square Shape \"Square Creature\"",
	"Square Shape \"Square Creature\"",
	"Circular Shape \"Perfect Circle Sculpting Art\"",
	"Circular Shape \"Perfect Circle Sculpting Art\"",
	"Circular Shape \"Circle Creature\"",
	"Circular Shape \"Circle Creature\"",
	"Linear Shape \"Line-Shaped Sculpting Art\"",
	"Linear Shape \"Line-Shaped Sculpting Art\"",
	"Linear Shape \"Linear Creature\"",
	"Linear Shape \"Linear Creature\"",
	"Haniwa \"Horse-and-Rider Idol Sculpting Art\"",
	"Haniwa \"Horse-and-Rider Idol Sculpting Art\"",
	"Haniwa \"Idol Creature\"",
	"Haniwa \"Idol Creature\"",
	"\"Oni-Shaped Sculpting Art\"",
	"\"Oni-Shaped Sculpting Art\"",
	"\"Oni-Shaped Sculpting Art\"",
	"\"Oni-Shaped Sculpting Art\"",
	"\"Geometric Creature\"",
	"\"Geometric Creature\"",
	"\"Geometric Creature\"",
	"\"Geometric Creature\"",
	"\"Idola Diabolus\"",
	"\"Idola Diabolus\"",
	"\"Idola Diabolus\"",
	"\"Idola Diabolus\"",
	// extra stage (89-101, 13 cards)
	"Bloody Battle \"Watershed of Blood\"",
	"Bloody Battle \"Hell Realm Sight-Line\"",
	"Bloody Battle \"All-Spirit Oniwatari\"",
	"Agile Skill \"Thrilling Shot\"",
	"Agile Skill \"Lightning Neigh\"",
	"Agile Skill \"Dense Crowd\"",
	"Agile Skill \"Beast Epidemicity\"",
	"Agile Skill \"Triangle Chase\"",
	"Agile Skill \"Black Pegasus Meteor Shot\"",
	"Agile Skill \"Muscle Explosion\"",
	"\"Follow Me, Unafraid\"",
	"\"Oni-Shaped Hoi Polloi\"",
	"\"Deeds of Devilish Beasts\"",
};

// Touhou 17 Music
const std::string th17_musicNames[19] = {
	"unused",
	"Silent Beast Spirits",
	"unused",
	"The Lamentations Known Only to Jizo",
	"Jelly Stone",
	"Lost River",
	"The Stone Baby and the Submerged Bovine",
	"Everlasting Red Spider Lily",
	"Seraphic Chicken",
	"Unlocated Hell",
	"Tortoise Dragon ~ Fortune and Misfortune",
	"Beast Metropolis",
	"Joutoujin of Ceramics",
	"Entrust the World to Idols ~ Idolatrize World",
	"Electric Heritage",
	"The Animals' Rest",
	"Return from the Underground",
	"The Shining Law of the Strong Eating the Weak",
	"Prince Shoutoku's Pegasus ~ Dark Pegasus",
};

// Touhou 18
const std::string th18_spellCardName[97] = {
	// stage 1 (1-8, 8 cards)
	"Beckon Sign \"Many Danmaku Guests\"",
	"Beckon Sign \"Many Danmaku Guests\"",
	"Beckon Sign \"Many Danmaku Guests\"",
	"Beckon Sign \"Many Danmaku Guests\"",
	"Beckon Sign \"Shoot Away Disaster, Beckon in Fortune\"",
	"Beckon Sign \"Shoot Away Disaster, Beckon in Fortune\"",
	"Beckon Sign \"Shoot Away Disaster, Beckon in Fortune\"",
	"Beckon Sign \"Shoot Away Disaster, Beckon in Fortune\"",
	// stage 2 (9-20, 12 cards)
	"Forest Sign \"Tree-Veiling Technology\"",
	"Forest Sign \"Tree-Veiling Technology\"",
	"Forest Sign \"Extreme Tree-Veiling Technology\"",
	"Forest Sign \"True Tree-Veiling Technology\"",
	"Forest Sign \"Innermost Forest Region\"",
	"Forest Sign \"Innermost Forest Region\"",
	"Forest Sign \"Extreme Innermost Forest Region\"",
	"Forest Sign \"True Innermost Forest Region\"",
	"Leaf Skill \"Green Spiral\"",
	"Leaf Skill \"Green Spiral\"",
	"Leaf Skill \"Green Cyclone\"",
	"Leaf Skill \"Green Tornado\"",
	// stage 3 (21-32, 12 cards)
	"Mountain Sign \"Heaven-Shaking Kumomagusa\"",
	"Mountain Sign \"Heaven-Shaking Kumomagusa\"",
	"Mountain Apparition \"Astonishing Kumomagusa\"",
	"Mountain Apparition \"Astonishing Kumomagusa\"",
	"Mountain Sign \"Usuyukisou Shining with Bewitching Light\"",
	"Mountain Sign \"Usuyukisou Shining with Bewitching Light\"",
	"Mountain Apparition \"Usuyukisou of Thronging Crowds of Youma\"",
	"Mountain Apparition \"Usuyukisou of Thronging Crowds of Youma\"",
	"Mountain Flower \"Komakusa of Massacre\"",
	"Mountain Flower \"Komakusa of Massacre\"",
	"Mountain Flower \"Mountain Queen of Massacre\"",
	"Mountain Flower \"Mountain Queen of Massacre\"",
	// stage 4 (33-44, 12 cards)
	"Orb Sign \"Rainbow Dragon Yin-Yang Orbs\"",
	"Orb Sign \"Rainbow Dragon Yin-Yang Orbs\"",
	"Orb Sign \"Rainbow Dragon Yin-Yang Orbs\"",
	"Orb Sign \"Yin-Yang Divine Orbs\"",
	"Jeweled General \"Queen of Yin-Yang Sphere\"",
	"Jeweled General \"Queen of Yin-Yang Sphere\"",
	"Queenly Gem \"Beyond the Rainbow Door\"",
	"Queenly Gem \"Beyond the Rainbow Door\"",
	"\"Yin-Yang Suffocation\"",
	"\"Yin-Yang Suffocation\"",
	"\"Yin-Yang Suffocation\"",
	"\"Yin-Yang Suffocation\"",
	// stage 5 (45-60, 16 cards)
	"Calamitous Star \"Dance of the Wildfire Started by a Star's Spark\"",
	"Calamitous Star \"Dance of the Wildfire Started by a Star's Spark\"",
	"Calamitous Star \"Wild Dance of the Wildfire Started by a Star's Spark\"",
	"Calamitous Star \"Wild Dance of the Wildfire Started by a Star's Spark\"",
	"Stellar Wind \"Dance of Dazzling Iridescence\"",
	"Stellar Wind \"Dance of Dazzling Iridescence\"",
	"Stellar Wind \"Wild Dance of Dazzling Iridescence\"",
	"Stellar Wind \"Wild Dance of Dazzling Iridescence\"",
	"Luminous Horse \"Dance of Sky-Racing Heavenly Steeds\"",
	"Luminous Horse \"Dance of Sky-Racing Heavenly Steeds\"",
	"Luminous Horse \"Wild Dance of Sky-Racing Heavenly Steeds\"",
	"Luminous Horse \"Wild Dance of Sky-Racing Heavenly Steeds\"",
	"Rainbow Illumination \"Clear and Tranquil Wind and Moon\"",
	"Rainbow Illumination \"Clear and Tranquil Wind and Moon\"",
	"Rainbow Illumination \"Clear and Tranquil Wind and Moon\"",
	"Rainbow Illumination \"Clear and Tranquil Wind and Moon\"",
	// stage 6 (61-84, 24 cards)
	"\"An Offering to the Ownerless\"",
	"\"An Offering to the Ownerless\"",
	"\"An Offering to the Ownerless\"",
	"\"An Offering to the Ownerless\"",
	"\"Danmaku Hoarder's Obsession\"",
	"\"Danmaku Hoarder's Obsession\"",
	"\"Danmaku Hoarder's Obsession\"",
	"\"Danmaku Hoarder's Obsession\"",
	"\"Bullet Market\"",
	"\"Bullet Market\"",
	"\"High-Density Bullet Market\"",
	"\"Danmaku Free Market\"",
	"\"Rainbow Ring of People\"",
	"\"Rainbow Ring of People\"",
	"\"Rainbow Ring of People\"",
	"\"Rainbow Ring of People\"",
	"\"Bullet Dominion\"",
	"\"Bullet Dominion\"",
	"\"Tyrannical Bullet Dominion\"",
	"\"Inhumane Bullet Dominion\"",
	"\"Asylum of Danmaku\"",
	"\"Asylum of Danmaku\"",
	"\"Asylum of Danmaku\"",
	"\"Asylum of Danmaku\"",
	// extra stage (85-97, 13 cards)
	"Fox Sign \"Fox Winder\"",
	"Kuda-gitsune \"Cylinder Fox\"",
	"Stellar Fox \"Dance of Heavenly Foxes and Dragon Stars\"",
	"Kodoku \"Cannibalistic Insect\"",
	"Kodoku \"Cave Swarmer\"",
	"Kodoku \"Sky Pendra\"",
	"Mining \"Ever-Accumulating Mine Dump\"",
	"Mining \"Mine Blast\"",
	"Mining \"Shield Method of the Youkai\"",
	"Oomukade \"Snake Eater\"",
	"Oomukade \"Dragon Eater\"",
	"\"Kodoku Gourmet\"",
	"\"Mushihime-sama's Resplendent and Restless Daily Life\"",
};

// Touhou 18 Music
const std::string th18_musicNames[] = {
	"unused",
	"A Rainbow Spanning Gensokyo",
	"unused",
	"A Shower of Strange Occurrences",
	"Kitten of Great Fortune",
	"The Cliff Hidden in Deep Green",
	"Banditry Technology",
	"The Perpetual Snow of Komakusa Blossoms",
	"Smoking Dragon",
	"The Obsolescent Industrial Remains",
	"Ore from the Age of the Gods",
	"The Long-Awaited Oumagatoki",
	"Starry Mountain of Tenma",
	"Lunar Rainbow",
	"Where Is That Bustling Marketplace Now ~ Immemorial Marketeers",
	"The Sunday After the Storm",
	"A Rainbow-Colored World",
	"The Great Fantastic Underground Railway Network",
	"The Princess Who Slays Dragon Kings",
};
